...
begin
	...
	dbms_output.put_line('Found ' || v_rows || ' rows');
	...
	dbms_output.put_line('Making Index on Loc, Hour, Date');
	v_timer := systimestamp;
	execute immediate 'create index idx_entries_scan on iot_entries(iot_loc, iot_date, iot_hour_of_day)';
...
begin
    for rec in (
		...
		from user_segments
		where segment_name in ('IOT_ENTRIES', 'IDX_ENTRIES_SCAN')
